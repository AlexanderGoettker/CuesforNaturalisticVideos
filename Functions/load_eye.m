function [xpos ypos events saccade blinks pupil Bad_Trial] = load_eye(log_file,t);
% Function to load the raw eye data and convert them into deg

global datapath conv Connector Blink_Error% Get the global data
Bad_Trial = 0;
...
% Goal is to have vector, where each row is an event (e.g. saccade or
% blink) and the first colum defines start and second colum end of it.
...
% Check blinks to make sure they fit in the vector
check = find(blinks>= length(xpos));
if ~isempty(check)
    blinks (check ) = length(xpos)-1;
    Bad_Trial = 1;
end
...
if ~isempty(blinks_raw)
    ...
    if ~isempty(blinks)
        for aa = 1: size(blinks,1)
            ...
            if blinks(aa,2)-blinks(aa,1)> 500
                Bad_Trial =1;
                Blink_Error=  Blink_Error+1;
                break
            end
        ...
        if max(blinks_raw) == length(xpos)
            ...
            if max(blinks_raw)-min(blinks_raw)> 500
                Bad_Trial =1;
                Blink_Error= Blink_Error+1;
            end
